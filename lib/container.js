...
class Container  {
  ...
  async run() {
    ...
    let {Configs} = this.specs;
    delete this.specs.Configs;
    ...
    if(Configs.length) {
      let payload = await this.stack.build_config_archive(Configs);

      let request =  {path : `/containers/${container.Id}/archive`, qs : {"path" : '/'}, headers : {'content-type' : 'application/x-tar'}};

      let extracted = await this.stack.request('PUT', request, payload);
      if(extracted.statusCode !== 200) {
        console.error(String(await drain(extracted)));
        throw `Count not attach config in container (${extracted.statusCode})`;
      }
    }
